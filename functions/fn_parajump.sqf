...
private _host = _this select 0;
private _caller = _this select 1;
private _id = _this select 2;
private _params = _this select 3;
private _typehalo = _params select 0;//true for all group, false for player only.
private _althalo = _params select 1;//altitude of halo jump
private _altchute = _params select 2;//altitude for autochute deployment
...
private _pos = clickpos;

if (_typehalo) then {
private _grp1 = group _caller;
...
private _bis_fnc_halo_action = _caller addaction ["<t color='#ff0000'>Fallschirm öffnen!</t>","A3\functions_f\misc\fn_HALO.sqf",[],1,true,true,"Ausstieg!"];
